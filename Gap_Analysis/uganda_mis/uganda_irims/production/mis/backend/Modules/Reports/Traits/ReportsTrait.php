	...
	function printPremisesCertificateLetter($request,$approvalGrant){
        ...
        if(!empty($approvalGrant) && $approvalGrant->decision_id == 1){
											...
											if($record){
												...
												$pdf->ln();
												$pdf->Cell(0,5,'LICENSE TO OPERATE '.strtoupper($record->premises_type),0,1,'');
												$pdf->ln();
												$pdf->SetFont('times','',10);
		...
		try{
						...
						if($row){
							...
							$org_info = $this->getOrganisationInfo();
								...
								//$pdf->Cell(0,1,'',0,2);
								$pdf->SetFont('times','',10);
								 $pdf->Cell(70,8,'Date of registration:',0,0);
								   $pdf->SetFont('times','B',10);
								 $pdf->Cell(100,8,$row->approval_date,0,1);
                                 $pdf->SetFont('times','',10);
								 $pdf->Cell(70,8,'Expiry date of registration:',0,0);
								   $pdf->SetFont('times','B',10);
								 $pdf->Cell(100,8,$row->expiry_date,0,1);
